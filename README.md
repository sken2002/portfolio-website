...
**Opening.** A title card plays once per session: the name wipes in, an oxide rule draws
beneath it, then the card lifts to reveal the page. Any input cuts it short, it is skipped
entirely under `prefers-reduced-motion`, and because JavaScript adds it a no-JS visitor never
sees it at all.
